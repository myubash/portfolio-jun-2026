...
export default function Skills() {
  ...
  return (
    <section id="skills" className="relative z-10 py-28 bg-[#111827] border-t border-b border-white/[0.08]">
      <div className="max-w-[1280px] mx-auto px-8">
        ...
        <div ref={header.ref} className={`transition-all duration-700 ${header.visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}>
          ...
          <h2 className="text-[clamp(28px,3.5vw,40px)] font-bold tracking-tight mb-4">What I Work With</h2>
        ...
        <div ref={grid.ref} className={`mt-12 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 transition-all duration-700 delay-150 ${grid.visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}>
          {skillGroups.map((group) => (
            <div key={group.title} className="bg-[#0B1120] border border-white/[0.08] rounded-2xl p-6">
              <div className="text-[12px] font-semibold uppercase tracking-[1.5px] text-[#94A3B8] mb-4">{group.title}</div>
              <div className="flex flex-wrap gap-2">
                {group.skills.map((skill) => (
                  <span key={skill} className="bg-white/[0.05] border border-white/[0.08] rounded-lg px-3 py-1.5 text-[13px] text-[#F8FAFC] hover:border-white/20 transition-colors duration-200">
                    {skill}
                  </span>
                ))}
              </div>
