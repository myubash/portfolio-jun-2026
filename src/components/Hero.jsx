...
export default function Hero() {
  ...
  return (
    <section id="hero" className="relative z-10 min-h-screen flex items-center pt-24 pb-16 px-0">
      <div className="max-w-[1280px] mx-auto px-5 sm:px-8 w-full">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16 items-center">
          ...
          >
            <div className="inline-flex items-center gap-2 bg-[#3B82F6]/10 border border-[#3B82F6]/25 rounded-full px-3 py-1.5 text-[12px] sm:text-[13px] text-[#3B82F6] mb-5">
              <span className="w-1.5 h-1.5 rounded-full bg-[#10B981] animate-pulse flex-shrink-0" />
              Open to new opportunities
            </div>

            <h1 className="text-[clamp(36px,8vw,68px)] font-bold leading-[1.08] tracking-[-1.5px] mb-5">
              I build things
              <br />
              for the{' '}
            ...
            <p className="text-[15px] sm:text-[17px] text-[#94A3B8] max-w-[480px] mb-8 leading-relaxed">
            ...
            <div className="flex gap-3 sm:gap-4 flex-wrap">
              <a
                href="#work"
                className="px-5 sm:px-7 py-3 sm:py-3.5 rounded-xl text-[14px] sm:text-[15px] font-semibold bg-gradient-to-r from-[#3B82F6] to-[#6366F1] text-white hover:-translate-y-0.5 hover:shadow-[0_8px_30px_rgba(59,130,246,0.4)] transition-all duration-200"
              >
                View My Work
              </a>
              <a
                href="#contact"
                className="px-5 sm:px-7 py-3 sm:py-3.5 rounded-xl text-[14px] sm:text-[15px] font-medium border border-white/[0.08] text-[#F8FAFC] hover:border-white/20 hover:bg-white/[0.05] transition-all duration-200"
              >
                Get in Touch
              </a>
            </div>

            {/* Mobile-only mini tech stack pills */}
            <div className="flex flex-wrap gap-2 mt-7 lg:hidden">
              {['React', 'Node.js', 'MongoDB', 'AWS', 'Web GIS', 'TypeScript'].map((t) => (
                <span
                  key={t}
                  className="bg-white/[0.04] border border-white/[0.08] rounded-lg px-3 py-1 text-[11px] text-[#94A3B8]"
                >
                  {t}
                </span>
              ))}
        ...
        <div
          ref={statsRef.ref}
          className={`mt-10 sm:mt-16 grid grid-cols-3 divide-x divide-white/[0.08] border border-white/[0.08] rounded-2xl overflow-hidden transition-all duration-700 delay-300 ${statsRef.visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}
        >
          {stats.map((s) => (
            <div key={s.label} className="bg-[#111827] py-5 sm:py-8 text-center">
              <div className="text-[24px] sm:text-[40px] font-bold leading-none bg-gradient-to-r from-[#3B82F6] to-[#6366F1] bg-clip-text text-transparent mb-1 sm:mb-2">
                {s.num}
              </div>
              <div className="text-[10px] sm:text-[14px] text-[#94A3B8] px-1 leading-tight">{s.label}</div>
            </div>
          ))}
        </div>
      </div>
